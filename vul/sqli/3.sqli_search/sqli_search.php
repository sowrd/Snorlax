...
try {
    if(isset($_POST['change'])) {
        if (!empty($_POST['username'])) {
            ...
            $sql_query = "SELECT id,username,email FROM member WHERE username like '%{$username}%'";
            ...
            if ($result->num_rows) {
                while($row = $result->fetch_assoc()){
                    ...
                    $email = $row['email'];
                    $html .= <<<str
id: {$id}<br />username: {$username}<br />email: {$email}<hr />
str;
                }
            } else {
                $html = <<<str
...
<div class="col-xl-8">
    <article class="lyear-arc">
        <div class="arc-header">
            <h2 class="arc-title"><a href="sqli_search.php">搜索型注入</a></h2>
            <ul class="arc-meta">
        ...
        <div class="arc-synopsis">
            <h6 style="color: black">请输入用户名进行查找!<br />如果记不住用户名，输入用户名的一部分搜索的试试看？</h6>
